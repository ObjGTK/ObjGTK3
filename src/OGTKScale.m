...
- (bool)getDrawValue
{
	return gtk_scale_get_draw_value(GTK_SCALE([self GOBJECT]));
}

- (bool)getHasOrigin
{
	return gtk_scale_get_has_origin(GTK_SCALE([self GOBJECT]));
}
...
{
	gtk_scale_set_draw_value(GTK_SCALE([self GOBJECT]), drawValue);
}

- (void)setHasOrigin:(bool)hasOrigin
{
	gtk_scale_set_has_origin(GTK_SCALE([self GOBJECT]), hasOrigin);
}
