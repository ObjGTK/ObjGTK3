...
- (id)initWithWidget:(OGTKWidget*)widget andFlags:(GtkEventControllerScrollFlags)flags
{
...
{
	return gtk_event_controller_scroll_get_flags(GTK_EVENT_CONTROLLER_SCROLL([self GOBJECT]));
}

- (void)setFlags:(GtkEventControllerScrollFlags)flags
{
	gtk_event_controller_scroll_set_flags(GTK_EVENT_CONTROLLER_SCROLL([self GOBJECT]), flags);
}
