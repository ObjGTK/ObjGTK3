...
{
	return gtk_scrolled_window_get_capture_button_press(GTK_SCROLLED_WINDOW([self GOBJECT]));
}
...
{
	return [[OGTKWidget alloc] initWithGObject:(GObject*)gtk_scrolled_window_get_hscrollbar(GTK_SCROLLED_WINDOW([self GOBJECT]))];
}

- (bool)getKineticScrolling
{
	return gtk_scrolled_window_get_kinetic_scrolling(GTK_SCROLLED_WINDOW([self GOBJECT]));
}
...
{
	return gtk_scrolled_window_get_overlay_scrolling(GTK_SCROLLED_WINDOW([self GOBJECT]));
}
...
{
	return gtk_scrolled_window_get_propagate_natural_height(GTK_SCROLLED_WINDOW([self GOBJECT]));
}

- (bool)getPropagateNaturalWidth
{
	return gtk_scrolled_window_get_propagate_natural_width(GTK_SCROLLED_WINDOW([self GOBJECT]));
}
...
{
	return [[OGTKWidget alloc] initWithGObject:(GObject*)gtk_scrolled_window_get_vscrollbar(GTK_SCROLLED_WINDOW([self GOBJECT]))];
}

- (void)setCaptureButtonPress:(bool)captureButtonPress
{
	gtk_scrolled_window_set_capture_button_press(GTK_SCROLLED_WINDOW([self GOBJECT]), captureButtonPress);
}
...
{
	gtk_scrolled_window_set_kinetic_scrolling(GTK_SCROLLED_WINDOW([self GOBJECT]), kineticScrolling);
}
...
{
	gtk_scrolled_window_set_overlay_scrolling(GTK_SCROLLED_WINDOW([self GOBJECT]), overlayScrolling);
}
...
{
	gtk_scrolled_window_set_propagate_natural_height(GTK_SCROLLED_WINDOW([self GOBJECT]), propagate);
}

- (void)setPropagateNaturalWidth:(bool)propagate
{
	gtk_scrolled_window_set_propagate_natural_width(GTK_SCROLLED_WINDOW([self GOBJECT]), propagate);
}
