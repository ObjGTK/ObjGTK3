...
{
	return gtk_flow_box_get_activate_on_single_click(GTK_FLOW_BOX([self GOBJECT]));
}
...
- (bool)getHomogeneous
{
	return gtk_flow_box_get_homogeneous(GTK_FLOW_BOX([self GOBJECT]));
}
...
- (void)insertWithWidget:(OGTKWidget*)widget andPosition:(gint)position
{
...
{
	gtk_flow_box_set_activate_on_single_click(GTK_FLOW_BOX([self GOBJECT]), single);
}
...
{
	gtk_flow_box_set_homogeneous(GTK_FLOW_BOX([self GOBJECT]), homogeneous);
}
