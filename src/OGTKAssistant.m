...
{
	return [[OGTKWidget alloc] initWithGObject:(GObject*)gtk_assistant_get_nth_page(GTK_ASSISTANT([self GOBJECT]), pageNum)];
}

- (bool)getPageComplete:(OGTKWidget*)page
{
	return gtk_assistant_get_page_complete(GTK_ASSISTANT([self GOBJECT]), [page WIDGET]);
}

- (bool)getPageHasPadding:(OGTKWidget*)page
{
	return gtk_assistant_get_page_has_padding(GTK_ASSISTANT([self GOBJECT]), [page WIDGET]);
}
...
{
	gtk_assistant_set_page_complete(GTK_ASSISTANT([self GOBJECT]), [page WIDGET], complete);
}

- (void)setPageHasPaddingWithPage:(OGTKWidget*)page andHasPadding:(bool)hasPadding
{
	gtk_assistant_set_page_has_padding(GTK_ASSISTANT([self GOBJECT]), [page WIDGET], hasPadding);
}
